...
class UploadTest(test_base.CdcRsyncTest):
  ...
  def test_single_unicode(self):
    ...
    nonascii_local_data_path = f'{self.local_base_dir}⛽⛽⛽⛽⛽⛽⛽⛽.dat'
    nonascii_remote_data_path = f'{self.remote_base_dir}⛽⛽⛽⛽⛽⛽⛽⛽.dat'
  ...
  def _do_test_no_empty_folders(self, compressed):
    ...
    """
    compressed_arg = '--compress' if compressed else None
    local_root_path = f'{self.local_base_dir}rootdir'
    remote_root_path = f'{self.remote_base_dir}rootdir/'
    ...
    self.assertTrue(
        utils.sha1_matches(
            local_root_path + '\\dir1\\file1_1.txt',
            f'{remote_root_path}dir1/file1_1.txt',
        ))
    self.assertTrue(
        utils.sha1_matches(
            local_root_path + '\\dir1\\file1_2.txt',
            f'{remote_root_path}dir1/file1_2.txt',
        ))
    self.assertTrue(
        utils.sha1_matches(
            local_root_path + '\\dir2\\file2_1.txt',
            f'{remote_root_path}dir2/file2_1.txt',
        ))
    self.assertTrue(
        utils.sha1_matches(local_root_path + '\\file0.txt',
                           f'{remote_root_path}file0.txt'))
    ...
    self.assertFalse(
        utils.does_directory_exist_remotely(f'{remote_root_path}dir1'))
    self.assertFalse(
        utils.does_directory_exist_remotely(f'{remote_root_path}dir2'))
  ...
  def _do_test_no_empty_folders_with_backslash(self, compressed):
    ...
    utils.create_test_file(local_root_path + 'dir1\\file1_2.txt', 1024)
    utils.create_test_file(local_root_path + 'dir2\\file2_1.txt', 1024)
    utils.create_test_file(f'{local_root_path}file0.txt', 1024)
    res = utils.run_rsync(local_root_path, self.remote_base_dir, compressed_arg,
                          '-r')
    self._assert_rsync_success(res)
    ...
    self.assertTrue(
        utils.sha1_matches(
            local_root_path + 'dir1\\file1_1.txt',
            f'{self.remote_base_dir}dir1/file1_1.txt',
        ))
    self.assertTrue(
        utils.sha1_matches(
            local_root_path + 'dir1\\file1_2.txt',
            f'{self.remote_base_dir}dir1/file1_2.txt',
        ))
    self.assertTrue(
        utils.sha1_matches(
            local_root_path + 'dir2\\file2_1.txt',
            f'{self.remote_base_dir}dir2/file2_1.txt',
        ))
    self.assertTrue(
        utils.sha1_matches(f'{local_root_path}file0.txt',
                           f'{self.remote_base_dir}file0.txt'))
    ...
    self.assertFalse(
        utils.does_directory_exist_remotely(f'{self.remote_base_dir}dir1'))
    self.assertFalse(
        utils.does_directory_exist_remotely(f'{self.remote_base_dir}dir2'))
  ...
  def test_upload_empty_file(self):
    ...
    empty_local_data_path = f'{self.local_base_dir}emptyfile.dat'
    empty_remote_data_path = f'{self.remote_base_dir}emptyfile.dat'
  ...
  def test_keep_file_permissions(self):
    ...
    ls_res = utils.get_ssh_command_output(f'ls -al {self.remote_data_path}')
    self.assertIn('-rw-r--r--', ls_res)

    # Add executable bit.
    utils.get_ssh_command_output(f'chmod a+x {self.remote_data_path}*')
    ls_res = utils.get_ssh_command_output(f'ls -al {self.remote_data_path}')
  ...
  def test_exclude_include_from(self):
    ...
    include_file = f'{self.local_base_dir}include.txt'
    with open(include_file, 'wt') as f:
      f.writelines(['file1.txt\n', 'folder3\\file5.txt'])

    exclude_file = f'{self.local_base_dir}exclude.txt'
  ...
  def test_files_from(self):
    ...
    sources_file = f'{self.local_base_dir}sources.txt'
  ...
  def test_sync_folder_when_remote_file_non_recursive(self):
    ...
    res = utils.run_rsync(f'{self.local_base_dir}foldertocopy',
                          self.remote_base_dir)
    self._assert_rsync_success(res)
  ...
  def _do_test_sync_file_when_remote_folder_non_empty(self, recursive):
    ...
    utils.get_ssh_command_output(f'mkdir -p {self.remote_data_path}')
    utils.get_ssh_command_output(
        f'mkdir -p {self.remote_base_dir} && touch {self.remote_data_path}/file1.txt'
    )
    ...
    self.assertTrue(
        utils.does_file_exist_remotely(f'{self.remote_data_path}/file1.txt'))
  ...
  def test_upload_from_dot(self):
    ...
    utils.create_test_file(f'{self.local_base_dir}file1.txt', 1024)
  ...
  def test_upload_from_dotdot(self):
    ...
    prev_cwd = os.getcwd()
    os.chdir(f'{self.local_base_dir}dir')
    try:
      # Uploading recursivly should pick up all files and dirs.
      res = utils.run_rsync('..', self.remote_base_dir, '-r')
      self.assertTrue(utils.files_count_is(res, missing=2, missing_dir=1))
  ...
  def test_existing(self):
    ...
    self.assertTrue(
        utils.files_count_is(
            ...
            extraneous_dir=1))
    self.assertTrue(
        utils.does_directory_exist_remotely(f'{remote_root_path}emptydir1'))
    self.assertFalse(
        utils.does_directory_exist_remotely(f'{remote_root_path}emptydir4'))
    self.assertFalse(
        utils.does_directory_exist_remotely(f'{remote_root_path}dir1/emptydir3'))
    self.assertTrue(
        utils.does_file_exist_remotely(f'{remote_root_path}dir1/file1_2.txt'))
    self.assertFalse(
        utils.does_file_exist_remotely(f'{remote_root_path}dir1/file1_3.txt'))
    ...
    self.assertFalse(
        utils.does_directory_exist_remotely(f'{remote_root_path}emptydir1'))
    self.assertFalse(
        utils.does_file_exist_remotely(f'{remote_root_path}dir2/file1_2.txt'))
  ...
  def test_copy_dest(self):
    ...
    copy_dest_dir = f'{self.remote_base_dir}copy_dest_dir'
  ...
  def test_upload_executables(self):
    ...
    utils.get_ssh_command_output(f'chmod -x {remote_exe_path} {remote_elf_path}')
  ...
  def _run(self, args):
    ...
    self.assertEqual(res.returncode, 0, f'Command failed: {str(res)}')
